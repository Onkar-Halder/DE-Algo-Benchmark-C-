...
float f = 0.5;
float cr = 0.7;
float lb = -5.00, ub = 10.00;

int main()
{
    ofstream fout, myfile;
    fout.open("Rosenbrock_excel.csv");
    myfile.open("Rosenbrock_graph.csv");
    float runMin[50];
    const int iter=200;
    for (int w = 0; w < 50; w++)
    {
        cout << endl<< "RUN-" << w + 1 << endl;
        float minCostArr[10];
        float arr[100][5];
        /*cout << endl
             << "Search Space" << endl;*/
        ...
        while (b <= iter-1)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                for (int j = 0; j < 5-1; j++)
                {
                    xp = pow((pow(arr1[i][j], 2)- pow(arr1[i][j+1],2)),2);
                    xtri = pow((arr1[i][j])-1, 2);
                    sum_f += (100*xp)+xtri;
                }
                costRes[i] = sum_f;
            }
            ...
            for (int p = 0; p < 10; p++)
            {
                ...
                //*****SELECTION****
                float Xoffcost;
                float xp = 0,xtri=0,sum_f=0;
                for (int j = 0; j < 5-1; j++)
                {
                    xp = pow((pow(arrXoff1[j], 2)- pow(arrXoff1[j+1],2)),2);
                    xtri = pow((arrXoff1[j])-1, 2);
                    sum_f += (100*xp)+xtri;
                }
                Xoffcost = sum_f;
    ...
    for (int i = 0; i < 50; i++)
    {
        cout << setprecision(3) << runMin[i] << "  ";
    }

    for (int i = 0; i < 50; i++)
    {
        fout << setprecision(3) << runMin[i] << endl;
    }
    fout.close();
