...
int main()
{
    ...
    fout.open("zakharov_excel.csv");
    myfile.open("Zakharov_graph.csv");
    float runMin[50];
    const int iter=200;
    for (int w = 0; w < 50; w++)
    {
        ...
        while (b <= iter-1)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                for (int j = 0; j < 5; j++)
                {
                    sum_f += pow(arr1[i][j], 2);
                    sum_s += 0.5 * j * arr1[i][j];
                }
                costRes[i] = sum_f + pow(sum_s, 2) + pow(sum_s, 4);
            }
            ...
            for (int p = 0; p < 10; p++)
            {
                ...
                for (int j = 0; j < 5; j++)
                {
                    sum_f1 += pow(arrXoff1[j], 2);
                    sum_s1 += 0.5 * j * arrXoff1[j];
                }
                Xoffcost = sum_f1 + pow(sum_s1, 2) + pow(sum_s1, 4);
