...
int main()
{
    ...
    fout.open("SphereBest2_excel.csv");
    myfile.open("SphereBest2_graph.csv");
    float runMin[50];
    const int iter=200;
    for (int w = 0; w < 50; w++)
    {
        ...
        while (b <= iter-1)
        {
            ...
            for (int p = 0; p < 10; p++)
            {
                ...
                for (int k = 0; k < 5; k++)
                {
                    arrBest[k] = arr1[cost_index][k];
                }
                float arrXoff2[5];
                for (int k = 0; k < 5; k++)
                {
                    arrXoff2[k] = arrBest[k] + (f * (arrXr[k] - arrXs[k]))+(f * (arrXt[k] - arrXu[k]));
                }
